      ...
      <main className="mx-auto max-w-[1280px] px-5 pb-14 pt-28 lg:px-10 lg:pb-20 lg:pt-32">
        ...
        {!loggedIn && (
          <div className="mt-9 rounded-2xl border border-dashed border-[#c9bba5] bg-[#f9f0df] px-6 py-16 text-center" data-testid="status-saved-needs-login">
            ...
            <h3 className="mt-5 font-display text-3xl text-[#183c44]">Log in to see your saved spots.</h3>
            <p className="mx-auto mt-2 max-w-[330px] text-sm leading-6 text-[#476269]">Your saved places are tied to your account so they follow you across devices.</p>
            <a href={`/login?next=${encodeURIComponent('/saved')}`} className="mt-6 inline-block rounded-full bg-[#183c44] px-5 py-3 text-xs font-bold uppercase tracking-[.12em] text-[#f9f0df] transition hover:bg-[#24515a]" data-testid="link-login-from-saved">Log in</a>
          </div>
        )}
        ...
        {loggedIn && isLoading && (
          <div className="mt-9 rounded-2xl border border-dashed border-[#c9bba5] bg-[#f9f0df] px-6 py-16 text-center" data-testid="status-saved-loading">
            <p className="font-display text-3xl text-[#183c44]">Loading your saved spots…</p>
          </div>
        )}
        {loggedIn && isError && (
        ...
        {loggedIn && !isLoading && !isError && savedListings.length === 0 && (
          <div className="mt-9 rounded-2xl border border-dashed border-[#c9bba5] bg-[#f9f0df] px-6 py-16 text-center" data-testid="status-saved-empty">
            ...
            <h3 className="mt-5 font-display text-3xl text-[#183c44]">Nothing saved yet.</h3>
            <p className="mx-auto mt-2 max-w-[330px] text-sm leading-6 text-[#476269]">Tap the bookmark icon on any place while browsing to keep it here.</p>
            <a href="/browse" className="mt-6 inline-block rounded-full bg-[#183c44] px-5 py-3 text-xs font-bold uppercase tracking-[.12em] text-[#f9f0df] transition hover:bg-[#24515a]" data-testid="link-browse-from-empty">Browse places</a>
          </div>
        )}
                ...
                <article key={listing.id} className="group relative flex min-h-[300px] flex-col overflow-hidden rounded-2xl border border-[#d7c9b4] bg-[#f9f0df] transition duration-300 hover:-translate-y-1 hover:shadow-lg" data-testid={`card-saved-${listing.id}`}>
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <button onClick={() => unsaveMutation.mutate(listing.id)} aria-label={`Remove ${listing.name} from saved`} className="rounded-full border border-[#e58c70] bg-[#e58c70] p-2 text-[#fff8ed] transition hover:bg-[#d67558]" data-testid={`button-unsave-${listing.id}`}>
          ...
          <div className="relative max-h-[90dvh] w-full max-w-[560px] overflow-y-auto rounded-t-3xl bg-[#f9f0df] p-6 text-[#183c44] shadow-2xl sm:rounded-3xl sm:p-8">
            ...
            {(selected.instagramUrl || selected.websiteUrl || ((selected.category === 'Restaurants' || selected.category === 'Cafes') && (selected.hasMenu || selected.menuUrl))) && (
              <div className="mt-4 flex flex-wrap gap-3 text-[11px] font-bold uppercase tracking-[.08em] text-[#183c44]">
                {selected.instagramUrl && (
                  <a href={selected.instagramUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 hover:text-[#e58c70]" data-testid="link-instagram-saved"><Instagram className="h-3.5 w-3.5" /> Instagram</a>
                )}
                {selected.websiteUrl && (
                  <a href={selected.websiteUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 hover:text-[#e58c70]" data-testid="link-website-saved"><Globe className="h-3.5 w-3.5" /> Website</a>
                )}
                {(selected.category === 'Restaurants' || selected.category === 'Cafes') && (selected.hasMenu || selected.menuUrl) && (
                  <a href={resolveImageSrc({ hasImage: selected.hasMenu, imageUrl: selected.menuUrl }, menuUrlFor(selected.id))!} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 hover:text-[#e58c70]" data-testid="link-menu-saved"><FileText className="h-3.5 w-3.5" /> View menu</a>
                )}
              </div>
            )}
            <div className="mt-5 flex items-center justify-between rounded-xl bg-[#e9dfcd] p-3" data-testid="widget-rate-place">
              <div>
                ...
                <p className="mt-1 text-xs font-semibold">{ratingInfo?.yourRating ? 'Your rating' : 'Rate this place'}</p>
              </div>
